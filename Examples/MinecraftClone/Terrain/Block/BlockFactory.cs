...
public static class BlockFactory
{
    ...
    public static BlockBase CreateBlock(Silk.NET.OpenGL.GL gl, BlockId type, Vector3 position, string name)
        => type switch
        {
            BlockId.Dirt => new Dirt(gl, position, name),
            BlockId.Stone => new Stone(gl, position, name),
            BlockId.Grass => new Grass(gl, position, name),
