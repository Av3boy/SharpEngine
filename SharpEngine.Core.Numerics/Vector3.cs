...
namespace SharpEngine.Core.Numerics
{
    ...
    public partial struct Vector3 : IEquatable<Vector3>, IFormattable
    {
        ...
        public static Vector3 Lerp(Vector3 value1, Vector3 value2, float amount)
        {
            if (Vector.IsHardwareAccelerated)
            {
                var firstInfluence = value1 * (1f - amount);
                var secondInfluence = value2 * amount;
                return firstInfluence + secondInfluence;
            }

            return new Vector3(value1.X + ((value2.X - value1.X) * amount),
        ...
        public static Vector3 Transform(Vector3 position, Matrix4x4 matrix)
            => new((position.X * matrix.M11) + (position.Y * matrix.M21) + (position.Z * matrix.M31) + matrix.M41,
