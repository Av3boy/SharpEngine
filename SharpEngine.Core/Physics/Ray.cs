...
namespace SharpEngine.Core.Physics;
...
public class Ray
{
    ...
    public bool IsGameObjectInView(Scene scene, out GameObject? intersectingObject, out Vector3 hitPosition, params Type[] allowedTypes)
    {
        ...
        var sceneNodes = scene.Root.GetObjectsOfType<GameObject>();
        ...
        {
            var currentPosition = Origin + (t * Direction);
    ...
    {
        var min = obj.Transform.Position - (obj.Transform.Scale / 2);
        var max = obj.Transform.Position + (obj.Transform.Scale / 2);
